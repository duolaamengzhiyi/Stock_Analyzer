...
- [X] GitHub 账号 OK，本仓库能 `git push`
- [X] Vercel 账号已注册（建议 GitHub 登录）
- [X] Supabase 账号已注册（**尚未** 创建项目）
- [X] Sealos 账号已登录控制台
- [X] DeepSeek API Key 已创建并安全保存（形如 `sk-...`）
...
- [X] Vercel Dashboard 里能看到 `stock-analyzer-final` 项目
- [X] 项目 Settings → Git 里显示了 `main` 分支与 GitHub 仓库名
- [X] 生产域名 URL 记下
...
- [X] Vercel 项目 Settings → Environment Variables 里有一堆 `SUPABASE_*` 和
      `POSTGRES_*` 变量
- [X] Supabase Dashboard 能打开对应项目
- [X] Supabase Auth → URL Configuration 的 Site URL / Redirect URLs 已配置
- [X] Supabase Auth → Providers → Email → Confirm email 已开启
...
- [X] `.env.local` 已拉取，内容非空
- [X] `pnpm db:migrate` 无错误执行
- [X] Supabase Dashboard → Table Editor 能看到 `profiles` / `stocks` /
      `stock_daily` 等业务表
- [X] `invite_codes` 表里有 `violet-everGarden` 一行
