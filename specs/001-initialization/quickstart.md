...
### 1.5 Kimi (Moonshot AI) API Key

**访问**：<https://platform.moonshot.cn>
**注册**：支持手机号注册（国内）。
**创建 Key**：登录后 → 右上角头像 → API Key 管理 → 新建 → 复制得到形如
`sk-xxxxxxxxx...` 的字符串。**立即保存在安全地方**，页面关闭后再也不会
完整显示。

> Kimi 首次注册赠送的免费 token 对 MVP 阶段（每日几次午评 / 晚评 / 预测）
> 完全够用。若用完可以充值最低 10 元。
...
2. 填写：
   ...
   - **Environment Variables**（一次性填入全部，见 research.md R18 Sealos 清单）：
     ...
     KIMI_API_KEY=sk-xxxxxxxx（阶段 1.5 拿到的）
...
**排查**：Kimi 免费额度每分钟有限，AI job 密集触发可能超限。解决：
- 确认 Scheduler 的 APScheduler job 不会并发执行（`max_instances=1`）
- 拉长 ai-forecast 的 prompt，把一次 128k 替代多次 8k
