...
Scheduler 承担（AKTools 数据抓取、Kimi 批量生成、Realtime 广播）。两者
...
- **Supabase pg_cron + pg_net 调用 Sealos AKTools**：概念优雅但 pg_net
  ...
  且 Kimi 调用逻辑要写在 PL/pgSQL 里或 Edge Function 里，分散维护。
...
## R3. Supabase Auth + 占位邮箱包装账号名

**Decision**: 注册时前端拿账号名 + 密码 → 服务端 Route Handler 把账号名
正则校验（`^[A-Za-z0-9_-]{3,20}$`，FR-008），小写规范化后包装成
`{username}@stock-analyzer.local` 调用 Supabase Auth 的 `signUp`；业务表
`profiles` 保存 `{ user_id, username, created_at }`。登录同理——前端拿账号名，
服务端反查 `profiles.username → auth.users.email → signInWithPassword`。

**Rationale**:

- 宪法硬约束使用 Supabase Auth；Supabase Auth 默认以 email 为主身份标识，
  占位邮箱是业内标准 workaround。
- `.local` TLD 是 RFC 6762 保留给 mDNS 的私有域名，100% 不可能冲突到真实
  邮件基础设施，邮件发送也不会出问题（Supabase 默认关掉 email confirm）。
...
- **完全自研认证**（bcrypt + JWT + cookie）：违反宪法硬约束，RLS 无法利用
  `auth.uid()`。
- **Supabase Auth 真实邮箱注册**：FR 要求账号名登录，用户不提供邮箱，
  不可行。
...
## R8. Kimi LLM 调用：Vercel 用户触发 vs Sealos 批量生成

**Decision**: 两条并列路径：

- **Sealos Scheduler**（批量 / 后台）：午评 / 晚评 / 未来预测 / 新闻总结 /
  预测推荐。用 openai SDK 指向 Kimi 兼容端点（`base_url =
  https://api.moonshot.cn/v1`），非 streaming（后台任务不在意 TTFB）。
- **Vercel Route Handler**（按需 / 前台）：个股介绍、个股分析（US7 / FR-090-
  092）。用 `openai` SDK + `stream: true`，Route Handler 返回 `ReadableStream`
  让前端用 SSE 渲染。Vercel Hobby 10s timeout 下足以传输完整响应（Kimi TTFB
  ~1s，流式输出边传边渲染，不占 timeout）。

共享 `lib/kimi/prompts.ts` 中的 prompt 模板（Python 侧通过
`sealos/scheduler/clients/kimi.py` 镜像同一组 prompt 字符串）。

**Rationale**:

- 按需调用放 Vercel 避免绕一圈 Sealos 的 HTTP 回调延迟。
- 批量调用放 Sealos 利用其无 timeout + 可重试的优势。
- Streaming 解决 Vercel Hobby 10s 限制（真正的 Kimi 响应耗时通常 3~8s，
  streaming 能在 1s 内开始输出）。

**Alternatives considered**:

- **所有 LLM 调用都在 Sealos**：用户按需调用要走 `Vercel → Sealos →
  Kimi → Sealos → Vercel` 两跳，增加延迟；否决。
...
## R15. Kimi Prompt 工程：五类生成任务的差异化

**Decision**: `lib/kimi/prompts.ts` 定义 6 个 prompt builder（与 Python 端
同名镜像）：
...
Kimi 回复 **必须** 是纯文本，不要求结构化输出，简化 schema 风险。

**Rationale**:

- Prompt 固定结构让下游的 FR-055 新鲜度判定 + FR-054 双时间戳呈现逻辑统一。
- 不要求 JSON 输出避免 Kimi 偶发的解析失败；纯文本更稳。
...
- 覆盖宪法原则二"最优先级"：业务逻辑函数 + Hook + Route Handler + Drizzle
  查询封装。
- MSW 在 Unit / Component 层一致拦截 Supabase / Kimi / AKTools HTTP。
...
# 站内 AI 功能的模型名
KIMI_MODEL_STOCK_INTRO=moonshot-v1-8k
KIMI_MODEL_STOCK_ANALYSIS=moonshot-v1-32k
```
...
KIMI_API_KEY=
KIMI_BASE_URL=https://api.moonshot.cn/v1
KIMI_MODEL_MIDDAY=moonshot-v1-32k
KIMI_MODEL_EVENING=moonshot-v1-32k
KIMI_MODEL_FORECAST=moonshot-v1-128k
KIMI_MODEL_NEWS_SUMMARY=moonshot-v1-32k
...
| R15 | Kimi prompt | 6 个 prompt builder，文本输出 |
...
1. **申请 Kimi API key**：到 https://platform.moonshot.cn 注册账号并创建
   API key，首次获得免费额度。
