...
2. **Sealos（Python FastAPI Scheduler）**：承载所有后台定时任务——拉 AKTools
   原始数据、写入 Supabase、调用 Kimi 生成午评 / 晚评 / 未来预测 / 新闻
...
- Vercel Serverless Function timeout：Hobby 10s / Pro 60s（Kimi 按需调用需用
  streaming 降低 TTFB）
- Supabase Free 层：500 MB 数据 + 2 GB 月流量 + 50k MAU + pg_cron 支持。本项目
  数据规模 ≤ 60MB，流量估算足够用 Free；若用户量起来改 Pro。
- Kimi token 窗口：moonshot-v1-8k / 32k / 128k。午评 / 晚评 / 新闻总结用 32k
  足够；未来预测若聚合多日新闻可能需要 128k，按 prompt 长度动态选择。
...
- 业务 Hook 与 Route Handler 单测覆盖；MSW 拦截 Supabase / Kimi / AKTools HTTP
...
- 路由切换使用 `next-nprogress-bar` 顶部进度条（对应 FR-051d）
...
- 所有 Server/Client 边界（`"use client"`、Server Action、Route Handler）
  顶部一行注释说明运行环境与身份校验假设
- 筛选算法模块（`lib/screens/`）每个函数说明窗口、阈值、来自哪条 FR
- Scheduler 里每个 job 顶部注释触发时刻、失败处理、下游影响

### 原则七 — 客户端状态与 Zustand 统一管理 → ✅ 通过

- 唯一根 store：`stores/useAppStore.ts`，组合四个 slice：
...
│   ├── auth/
│   │   ├── register/route.ts             # 邀请码 + 账号名 + 密码（FR-002, FR-008）
...
│   └── auth.ts            # 注册 / 登录（占位邮箱包装）
├── kimi/
│   ├── client.ts          # 指向 Kimi OpenAI 兼容端点
│   └── prompts.ts         # midday / evening / forecast / news-summary / stock-intro / stock-analysis prompt 模板
├── aktools/
│   └── client.ts          # 仅 Next.js 侧用到的轻量调用（股票搜索 name/code 查询）
├── market-calendar/
│   ├── is-trading-day.ts  # (market, date) → boolean，读 market_calendar 表
│   └── last-trading-day.ts
├── screens/
│   ├── launching-soon.ts  # FR-040 算法（在 Scheduler 侧也复用同一份逻辑——通过把核心函数抽到此处）
│   └── main-uptrend.ts    # FR-041 算法
├── realtime/
...
│   └── realtimeSlice.ts   # Supabase Realtime 连接状态 + 断连重连（FR-10A）
