...
版本变更: 1.0.1 → 1.1.0
升级原因: MINOR 级改动 ——
  (a) 显著扩展原则二的测试指引，明确测试优先级、RSC 与 MSW 策略；
  (b) 新增原则六"新手友好的关键注释"，要求在关键步骤与复杂逻辑
      处添加简短注释。
未删除或重新定义其它原则；治理规则与技术栈约束保持不变。

修改的原则:
- 二、测试驱动开发（不可妥协）—— 扩展，新增"测什么 / 不测什么"的
  明确指引与 MSW 使用约定。
...
- ⚠ .specify/templates/plan-template.md —— Constitution Check 仍是
  占位符；后续需要把下列 **六** 条原则展开成具体的门禁清单，
  并补充 MSW / 注释相关的检查项。
- ⚠ .specify/templates/tasks-template.md —— 模板把测试标注为
  OPTIONAL；本项目按原则二视为强制，且测试任务描述中应体现
  "优先测试业务逻辑 / Hook / API 路由"的分层。
...
**严禁** 在尚未就位的数据上阻塞主线程。本原则通过以下硬性规则落地：
...
**版本**: 1.1.0 | **批准日期 (Ratified)**: 2026-04-22 | **最近修订 (Last Amended)**: 2026-04-22
