...
================================
版本变更: 1.1.1 → 1.1.2
升级原因: PATCH 级改动 —— 在原则五"交互即时反馈"规则下，追加
第 (d) 种可选落地手段："页面顶部全局加载进度条"，用于跨路由 /
跨区域的长耗时请求场景，作为 `useTransition` / `useOptimistic` /
显式 pending-disabled 态之外的补充选择。原则、治理规则、技术栈
约束均未变化，仅扩充了实现手段清单。

修改的原则:
- 五、性能与响应感知 —— "交互即时反馈"规则新增落地方式 (d)：
  页面顶部全局进度条（例如 nprogress / next-nprogress-bar）。
...
- ⚠ .specify/templates/plan-template.md —— Constitution Check 仍是
  占位符；原则五的"交互即时反馈"门禁需要在后续明确写入检查项。
- ⚠ .specify/templates/tasks-template.md —— 涉及异步交互的用户故事，
  其 UI 组件任务应默认生成一条"pending 态测试"子任务。
...
- **Spec-Kit 流程**：所有非琐碎的功能 **必须** 走
  ...
  **必须** 针对本宪法的五条原则逐条给出评估结论；出现违反时必须
  在 plan 的 Complexity Tracking 中登记理由。
...
**版本**: 1.1.2 | **批准日期 (Ratified)**: 2026-04-22 | **最近修订 (Last Amended)**: 2026-04-22
